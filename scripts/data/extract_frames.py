...
# Xóa toàn bộ ảnh cũ trong folder đích trước khi extract lại
# (mỗi folder class chỉ xóa 1 lần, dù có nhiều video cùng class)
...
def clear_images(folder):
    """Xóa tất cả ảnh trong folder, giữ nguyên thư mục."""
    if not os.path.exists(folder):
        return 0
    removed = 0
...
def print_dataset_summary():
    ...
    if all_ok:
        print("\n  ✅ Dataset đủ! Chạy tiếp: python scripts/data/review_data.py")
    else:
...
if __name__ == "__main__":
    ...
    if CLEAN_BEFORE_EXTRACT:
        seen_dirs = set()
        cleared = {}
        for _, output_dir in active:
            if output_dir in seen_dirs:
                continue
            seen_dirs.add(output_dir)
            n = clear_images(output_dir)
            if n:
                cleared[output_dir] = n
        if cleared:
            print("🗑️  Đã xóa ảnh cũ trước khi extract lại:")
            for folder, n in sorted(cleared.items()):
                print(f"   {folder}: {n} ảnh")
            print()
